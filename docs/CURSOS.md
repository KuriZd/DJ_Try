...
| Cualquier usuario activo autenticado | Consultar el catalogo activo, inscribirse y acceder al contenido de sus cursos activos. Consultar su propio avance y descargar sus certificados. |
| Anonimo | Sin acceso a cursos, inscripciones, avances ni certificados. |
...
| Metodo | Ruta | Uso |
| --- | --- | --- |
| GET / POST | `cursos/` | Catalogo / crear curso. El catalogo muestra metadatos; no expone el temario. |
| GET / PUT / PATCH / DELETE | `cursos/{id}/` | Consultar curso inscrito o administrado / editar / eliminar. |
| POST | `cursos/{id}/inscribir/` | Inscribirse; sin cuerpo. 201 al crear, 200 si ya existe. |
| GET | `cursos/{id}/lecciones/` | Temario ordenado; exige inscripcion o permiso de gestion. |
| GET / POST | `lecciones/` | Listar lecciones accesibles / crear. Filtro opcional `?curso={uuid}`. |
...
```json
{
  "titulo": "Introduccion a la seguridad",
  "descripcion": "Conceptos y procedimientos basicos",
  "imagen": "https://example.com/portadas/seguridad.jpg",
  "categoria": "Seguridad",
...
2. Cargar y confirmar el video usando el flujo existente:
`POST videos/upload/`, PUT directo a S3 y `POST videos/{id}/confirm/`.

3. Crear la leccion con `POST lecciones/`:
...
El orden es un entero no negativo, unico dentro del curso. Para intercambiar
ordenes, usar temporalmente un numero libre. Una leccion no puede trasladarse
a otro curso. El archivo permanece en S3; no se guardan URLs firmadas en la BD.

4. Publicar con `PATCH cursos/{id}/` y `{"activo": true}`.
5. El alumno se inscribe, consulta el temario y solicita la reproduccion del video.
6. Al terminar, envia `POST lecciones/{id}/progreso/` con `{"visto": true}`.
La respuesta incluye `progreso` e `inscripcion`, con el porcentaje actualizado.
7. Al completar todas las lecciones activas, aparece automaticamente un certificado
...
- Crear, activar, desactivar o eliminar lecciones por API recalcula todas las
  inscripciones del curso. Sustituir su video reinicia el progreso de esa leccion.
...
- DELETE de un curso elimina sus lecciones, inscripciones, avances y certificados.
